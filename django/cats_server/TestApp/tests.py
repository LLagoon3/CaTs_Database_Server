...
data = {'': ['2015년', '2016년', '2017년', '2018년', '2019년', '2020년'],
...
x_axis = ['first', 'second', 'third', 'fourth']
plt.figure(figsize=(10, 6))
for i in range(0, 6):
    tmp, label = list(), str()
    for key, val in data.items():
        if key == '':label = val[i][:-1]
        else: tmp.append(val[i])
    plt.plot(x_axis, tmp, marker='o', label = label)
